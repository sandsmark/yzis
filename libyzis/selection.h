...
{
public:
    YBound(const YBound &bound) :
        mPos(bound.pos())
    {
        mOpen = bound.opened();
    }
    explicit YBound(const YCursor pos, bool open = false) :
        mPos(pos)
    {
        mOpen = open;
    }
...
public:
    YInterval(const YBound &from, const YBound &to) :
        mFrom(from), mTo(to)
    {
    }
    YInterval(const YCursor from, const YCursor to) :
        mFrom(from), mTo(to)
    {
    }
    YInterval(const YBound &from, const YCursor to) :
        mFrom(from), mTo(to)
    {
    }
    YInterval(const YCursor from, const YBound &to) :
        mFrom(from), mTo(to)
    {
    }
    YInterval(const QRect &r) :
        mFrom(YCursor(r.left(), r.top())), mTo(YCursor(r.right(), r.bottom()))
    {
    }
    YInterval() :
        mFrom(YCursor(0, 0)), mTo(YCursor(0, 0), true)
    {
    }
