...
#define DUMP_BOUND(bound) \
    (bound).pos().x() << " " << (bound).pos().y() << " " << ((bound).closed() ? 1 : 0)
                stream << e->type << " " << DUMP_BOUND(e->interval.from()) << " " << DUMP_BOUND(e->interval.to()) << Qt::endl;

                for (int i = 0; i < e->data.count(); ++i) {
                    stream << " " << e->data.at(i) << Qt::endl;
                }
...
#endif
        QTextStream stream(&f);
        stream << "WARNING : do not edit, this file is a temporary file created by Yzis and used to recover files in case of crashes" << Qt::endl
               << Qt::endl;
        stream << "Generated by Yzis " << VERSION_CHAR << Qt::endl;
        stream << "Edited file: " << mParent->fileName() << Qt::endl;
        stream << "Creation date: " << QDateTime::currentDateTime().toString() << Qt::endl;
        // XXX stream << "Process ID: " << QString::number( getpid() ) << Qt::endl;
        stream << Qt::endl
               << Qt::endl
               << Qt::endl;
        f.close();
