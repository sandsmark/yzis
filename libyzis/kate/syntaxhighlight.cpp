...
public:
    ...
    virtual bool lineContinue() const
    {
        return false;
    }
    ...
    virtual YzisHlItem *clone(const QStringList *)
    {
        return this;
    }
...
static YzisHlItemData::ItemStyles getDefStyleNum(QString name)
{
    ...
    } else if(name == "dsKeyword") {
        return YzisHlItemData::dsKeyword;
    } else if(name == "dsDataType") {
        return YzisHlItemData::dsDataType;
    } else if(name == "dsDecVal") {
        return YzisHlItemData::dsDecVal;
    } else if(name == "dsBaseN") {
        return YzisHlItemData::dsBaseN;
    } else if(name == "dsFloat") {
        return YzisHlItemData::dsFloat;
    } else if(name == "dsChar") {
        return YzisHlItemData::dsChar;
    } else if(name == "dsString") {
        return YzisHlItemData::dsString;
    } else if(name == "dsComment") {
        return YzisHlItemData::dsComment;
    } else if(name == "dsOthers") {
        return YzisHlItemData::dsOthers;
    } else if(name == "dsAlert") {
        return YzisHlItemData::dsAlert;
    } else if(name == "dsFunction") {
        return YzisHlItemData::dsFunction;
    } else if(name == "dsRegionMarker") {
        return YzisHlItemData::dsRegionMarker;
    } else if(name == "dsError") {
        return YzisHlItemData::dsError;
    }
                             ...
                             int _fallthroughContext, bool _dynamic, bool _noIndentationBasedFolding)
{
    ...
    if(_noIndentationBasedFolding) {
        dbg() << QString("**********************_noIndentationBasedFolding is TRUE*****************") << endl;
    }
}
...
void YzisHighlighting::getYzisHlItemDataList(uint schema, YzisHlItemDataList &list)
{
    ...
    for(int ab = 0 ; ab < list.size() && (p = list.at(ab)) != 0L; ++ab) {
        ...
        if(s.count() > 0) {
            ...
            if(!tmp.isEmpty()) {
                p->setStrikeOut(tmp != "0");
            }

            tmp = s[6];

            if(!tmp.isEmpty()) {
                p->setUnderline(tmp != "0");
            }
...
void YzisHighlighting::addToYzisHlItemDataList()
{
    ...
    while(YzisHlManager::self()->syntax->nextGroup(data)) {
        ...
        if(!color.isEmpty()) {
            newData->setTextColor(YColor(color));
        }

        if(!selColor.isEmpty()) {
            newData->setSelectedTextColor(YColor(selColor));
        }

        if(!bold.isEmpty()) {
            newData->setBold(IS_TRUE(bold));
        }

        if(!italic.isEmpty()) {
            newData->setItalic(IS_TRUE(italic));
        }

        // new attributes for the new rendering view
        if(!underline.isEmpty()) {
            newData->setUnderline(IS_TRUE(underline));
        }

        if(!strikeOut.isEmpty()) {
            newData->setStrikeOut(IS_TRUE(strikeOut));
        }

        if(!bgColor.isEmpty()) {
            newData->setBGColor(YColor(bgColor));
        }

        if(!selBgColor.isEmpty()) {
            newData->setSelectedBGColor(YColor(selBgColor));
        }
        ...
        QStringList *ContextNameList)
{
    ...
    if(dataname == "keyword") {
        ...
        tmpItem = (new YzisHlFloat(attr, context, regionId, regionId2));
    } else if(dataname == "Int") {
        tmpItem = (new YzisHlInt(attr, context, regionId, regionId2));
    } else if(dataname == "DetectChar") {
        tmpItem = (new YzisHlCharDetect(attr, context, regionId, regionId2, chr));
    } else if(dataname == "Detect2Chars") {
        tmpItem = (new YzisHl2CharDetect(attr, context, regionId, regionId2, chr, chr1));
    } else if(dataname == "RangeDetect") {
        tmpItem = (new YzisHlRangeDetect(attr, context, regionId, regionId2, chr, chr1));
    } else if(dataname == "LineContinue") {
        tmpItem = (new YzisHlLineContinue(attr, context, regionId, regionId2));
    } else if(dataname == "StringDetect") {
        tmpItem = (new YzisHlStringDetect(attr, context, regionId, regionId2, stringdata, insensitive));
    } else if(dataname == "AnyChar") {
        tmpItem = (new YzisHlAnyChar(attr, context, regionId, regionId2, stringdata));
    } else if(dataname == "RegExpr") {
        tmpItem = (new YzisHlRegExpr(attr, context, regionId, regionId2, stringdata, insensitive, minimal));
    } else if(dataname == "HlCChar") {
        tmpItem = (new YzisHlCChar(attr, context, regionId, regionId2));
    } else if(dataname == "HlCHex") {
        tmpItem = (new YzisHlCHex(attr, context, regionId, regionId2));
    } else if(dataname == "HlCOct") {
        tmpItem = (new YzisHlCOct(attr, context, regionId, regionId2));
    } else if(dataname == "HlCFloat") {
        tmpItem = (new YzisHlCFloat(attr, context, regionId, regionId2));
    } else if(dataname == "HlCStringChar") {
        tmpItem = (new YzisHlCStringChar(attr, context, regionId, regionId2));
    } else if(dataname == "DetectSpaces") {
        tmpItem = (new YzisHlDetectSpaces(attr, context, regionId, regionId2));
    } else if(dataname == "DetectIdentifier") {
        tmpItem = (new YzisHlDetectIdentifier(attr, context, regionId, regionId2));
    } else {
...
int YzisHighlighting::getIdFromString(QStringList *ContextNameList, QString tmpLineEndContext, /*NO CONST*/ QString &unres)
{
    ...
    if((tmpLineEndContext == "#stay") || (tmpLineEndContext.simplified().isEmpty())) {
        ...
        if(!embeddedHls.contains(tmp)) {
            embeddedHls.insert(tmp, YzisEmbeddedHlInfo());
        }
        ...
        for(; it != end; ++it) {
            if(!it.value().loaded) { // we found one, we still have to load
                ...
                if(identifierToUse.isEmpty()) {
                    dbg() << "OHOH, unknown highlighting description referenced" << endl;
                }
...
void YzisHighlighting::handleYzisHlIncludeRules()
{
    ...
    for(; it != end;) {
        if((*it)->incCtx == -1) { // context unresolved ?
            ...
            ++it;    //nothing to do, already resolved (by the cross defintion reference resolver
        }
    }
...
void YzisHlManager::getDefaults(uint schema, YzisAttributeList &list)
{
    ...
    for(uint z = 0; z < defaultStyles(); z++) {
        ...
        if(!s.isEmpty()) {
            ...
            if(!tmp.isEmpty()) {
                i->setStrikeOut(tmp != "0");
            }

            tmp = s[5];

            if(!tmp.isEmpty()) {
                i->setUnderline(tmp != "0");
            }
