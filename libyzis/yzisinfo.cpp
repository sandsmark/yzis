...
void YInfo::write()
{
    ...
    if (mYzisinfo.open(QIODevice::WriteOnly)) {
        ...
        // Write header
        write << "# This yzisinfo file was generated by Yzis " << VERSION_CHAR << "." << Qt::endl;
        write << "# You may edit it if you're careful!" << Qt::endl;
        write << Qt::endl;
        // Write whether hlsearch is on or off
        write << "# Set hlsearch on or off:" << Qt::endl;
        write << "hlsearch ";
        ...
        write << Qt::endl;
        write << "# Command Line History (oldest to newest):" << Qt::endl;
        saveExHistory(write);
        write << Qt::endl;
        write << "# Search String History (oldest to newest):" << Qt::endl;
        saveSearchHistory(write);
        write << Qt::endl;
        write << "# Position to start at when opening file (oldest to newest):" << Qt::endl;
        saveStartPosition(write);
        write << Qt::endl;
        write << "# Jump list (oldest to newest):" << Qt::endl;
        saveJumpList(write);
        write << Qt::endl;
        write << "# Registers:" << Qt::endl;
...
void YInfo::saveStartPosition(QTextStream &write)
{
    ...
    for (int i = start; i < end; ++i) {
        ...
        write << (mStartPosition[i])->filename() << Qt::endl;
    }
}
...
void YInfo::saveJumpList(QTextStream &write)
{
    ...
    for (int i = start; i < end; ++i) {
        ...
        write << " ";
        write << mJumpList[i]->filename() << Qt::endl;
    }
}
...
void YInfo::saveRegistersList(QTextStream &write)
{
    ...
    for (int i = 0; i < list.size(); ++i) {
        ...
        if (contents.size() >= 3) {
            write << "LINE  " << contents.size() - 2 << Qt::endl;
        } else {
            write << "CHAR  " << contents.size() << Qt::endl;
        }
